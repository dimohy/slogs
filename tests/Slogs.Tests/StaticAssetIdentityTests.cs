...
public sealed class StaticAssetIdentityTests
{
    ...
    public void FirstScreenHeaderUsesMeaningRecallAndFlowToolLabels()
    {
        ...
        Assert.Contains("Match=\"NavLinkMatch.All\">대표 단서</NavLink>", navMenu);
        Assert.Contains("href=\"/tag\" Match=\"NavLinkMatch.All\">전체 단서</NavLink>", navMenu);
        ...
        Assert.Contains("반응 단서", navMenu);
        Assert.Contains("반응 로그 시리즈", navMenu);
        Assert.Contains("내 단서", navMenu);
        Assert.Contains("내 로그 시리즈", navMenu);
        Assert.Contains("공개 단서가 준비 중입니다.", navMenu);
        Assert.Contains("공개 로그 시리즈가 아직 없습니다.", navMenu);
        Assert.Contains("내 단서가 아직 없습니다.", navMenu);
        ...
        Assert.DoesNotContain("title=\"흐름 메뉴\"", mainLayout);
        Assert.DoesNotContain("aria-label=\"지식 로그 흐름 메뉴", mainLayout);
        Assert.DoesNotContain(">흐름 메뉴</span>", mainLayout);
        Assert.DoesNotContain("운영 흐름 모드", mainLayout);
        Assert.DoesNotContain("운영 흐름으로 전환", mainLayout);
        Assert.DoesNotContain("슬로거 흐름으로 돌아가기", mainLayout);
        Assert.DoesNotContain("aria-label=\"지식 로그 흐름 메뉴\"", navMenu);
        ...
        Assert.DoesNotContain(">전체 단서 흐름</NavLink>", navMenu);
        Assert.DoesNotContain(">슬로거</NavLink>", navMenu);
        Assert.DoesNotContain(">로그 시리즈 흐름</NavLink>", navMenu);
        Assert.DoesNotContain("인기 단서", navMenu);
        Assert.DoesNotContain("인기 로그 시리즈", navMenu);
        Assert.DoesNotContain("내 단서 흐름", navMenu);
        Assert.DoesNotContain("반응 단서 흐름", navMenu);
        Assert.DoesNotContain("내 로그 시리즈 흐름", navMenu);
        Assert.DoesNotContain("반응 로그 시리즈 흐름", navMenu);
    ...
    public void UserFacingFailureMessagesUseLogAndRecallWording()
    {
        ...
        Assert.Contains("Public-sharing node", postMcpTools);
        Assert.Contains("Public-sharing node time", postMcpTools);
    ...
    public void AdminUserFiltersUseClueLanguageInsteadOfGenericSearch()
    {
        ...
        Assert.DoesNotContain("aria-label=\"운영 슬로거 흐름 보기\"", adminUsersPage);
    ...
    public void LlmWikiUsageGuideFramesToolNamesAsRecallFlow()
    {
        ...
        Assert.Contains("자연어로 남긴 판단 단서는 제목, 태그, 요청 흐름, 본문을 함께 기억 신호로 엮습니다.", llmWikiGuidePage);
        ...
        Assert.Contains("복사한 Agent 회상 지침은 Agent 회상 연결 키 입력과 전역/프로젝트/현재 세션 범위 선택을 안내하고", llmWikiGuidePage);
        ...
        Assert.Contains("언어별 Agent 회상 지침으로 제공합니다.", llmWikiGuidePage);
        ...
        Assert.Contains("언어별 Agent 회상 지침", llmWikiGuidePage);
        Assert.Contains("복사되는 지침은 선택한 언어 하나만 포함합니다.", llmWikiGuidePage);
        Assert.Contains("한국어 회상 지침", llmWikiGuidePage);
        ...
        Assert.Contains("한국어 회상 지침이 복사되었습니다.", llmWikiGuidePage);
        ...
        Assert.DoesNotContain("자연어로 남긴 판단 단서는 제목, 태그, 프롬프트, 본문을 함께 기억 신호로 엮습니다.", llmWikiGuidePage);
    ...
    public void HomeFeedAndPostDetailAuthorActionsUseRelationshipFlowLanguage()
    {
        ...
        Assert.Contains(">관계 흐름</a>", homePage);
