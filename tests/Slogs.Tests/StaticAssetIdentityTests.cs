...
public sealed class StaticAssetIdentityTests
{
    ...
    public void FirstScreenHeaderUsesMeaningRecallAndFlowToolLabels()
    {
        ...
        Assert.Contains("사람과 AI가 이어 쓰는 지식 로그", mainLayout);
        Assert.Contains("placeholder=\"검색\"", mainLayout);
        Assert.Contains("aria-label=\"전역 검색어 입력\"", mainLayout);
        Assert.Contains("title=\"검색\"", mainLayout);
        Assert.Contains("aria-label=\"지식 로그 도구\"", mainLayout);
        Assert.Contains("href=\"/me\">내 지식 로그", mainLayout);
        Assert.Contains("aria-label=\"지식 로그 메뉴 열기\"", mainLayout);
        Assert.Contains("aria-label=\"지식 로그 메뉴 닫기\"", mainLayout);
        Assert.Contains(">메뉴</span>", mainLayout);
        Assert.Contains("aria-label=\"지식 로그 메뉴\"", navMenu);
        Assert.Contains(">공개</div>", navMenu);
        Assert.Contains("Match=\"NavLinkMatch.All\">대표 태그</NavLink>", navMenu);
        Assert.Contains("href=\"/tag\" Match=\"NavLinkMatch.All\">태그</NavLink>", navMenu);
        Assert.Contains("href=\"/writer\" Match=\"NavLinkMatch.All\">슬로거 홈</NavLink>", navMenu);
        Assert.Contains("href=\"/series\" Match=\"NavLinkMatch.All\">로그 시리즈</NavLink>", navMenu);
        Assert.Contains("인기 태그", navMenu);
        Assert.Contains("반응 로그 시리즈", navMenu);
        Assert.Contains("내 태그", navMenu);
        Assert.Contains("내 로그 시리즈", navMenu);
        Assert.Contains("태그가 준비 중입니다.", navMenu);
        Assert.Contains("공개 로그 시리즈가 아직 없습니다.", navMenu);
        Assert.Contains("내 태그가 아직 없습니다.", navMenu);
        Assert.Contains("내 로그 시리즈가 아직 없습니다.", navMenu);
        Assert.Contains("운영 모드", mainLayout);
        Assert.Contains("시작 슬로거 @", mainLayout);
        Assert.Contains("운영으로 전환", mainLayout);
        Assert.Contains("슬로거로 돌아가기", mainLayout);
        Assert.Contains("지식 로그 연결이 잠시 끊겼습니다.", mainLayout);
        Assert.Contains("다시 연결", mainLayout);
        Assert.Contains("알림 닫기", mainLayout);
        Assert.Contains("연결됨", reconnectModal);
        Assert.Contains("다시 연결 중", reconnectModal);
        Assert.Contains("연결 실패", reconnectModal);
        Assert.Contains("연결 일시 중지", reconnectModal);
        Assert.Contains("복구 실패", reconnectModal);
        Assert.Contains("재개", reconnectModal);
        Assert.Contains(">운영</div>", navMenu);
        Assert.DoesNotContain("href=\"/me\">내 로그", mainLayout);
        Assert.DoesNotContain("placeholder=\"검색어\"", mainLayout);
        ...
        Assert.DoesNotContain("aria-label=\"지식 로그 메뉴", mainLayout);
        Assert.DoesNotContain(">메뉴</span>", mainLayout);
        Assert.DoesNotContain("관리자 모드", mainLayout);
        Assert.DoesNotContain("관리자으로 전환", mainLayout);
        Assert.DoesNotContain("슬로거으로 돌아가기", mainLayout);
        Assert.DoesNotContain("aria-label=\"지식 로그 메뉴\"", navMenu);
        ...
        Assert.DoesNotContain(">추천 태그</NavLink>", navMenu);
        Assert.DoesNotContain(">태그</NavLink>", navMenu);
        Assert.DoesNotContain(">슬로거</NavLink>", navMenu);
        Assert.DoesNotContain(">로그 시리즈</NavLink>", navMenu);
        Assert.DoesNotContain("인기 태그", navMenu);
        Assert.DoesNotContain("인기 로그 시리즈", navMenu);
        Assert.DoesNotContain("내 태그", navMenu);
        Assert.DoesNotContain("인기 태그", navMenu);
        Assert.DoesNotContain("내 로그 시리즈", navMenu);
        Assert.DoesNotContain("반응 로그 시리즈", navMenu);
    ...
    public void GlobalMeaningRecallAcceptsKoreanFlowPrefixes()
    {
        ...
        {
            Assert.Contains("[\"tag:\", \"태그:\"]", source);
            Assert.Contains("[\"writer:\", \"슬로거:\"]", source);
            Assert.Contains("[\"series:\", \"시리즈:\", \"로그 시리즈:\"]", source);
            Assert.Contains("TryReadRecallPrefix", source);
        }

        Assert.Contains("IsRecallPrefixOnly(trimmed, [\"tag:\", \"태그:\", \"writer:\", \"슬로거:\", \"series:\", \"시리즈:\", \"로그 시리즈:\"])", homePage);
        ...
        Assert.Contains("($\"태그: {tagValue}\", () => ApiClient.GetByTagAsync(tagValue))", homePage);
    ...
    public void SeedDefaultBiosUseKnowledgeLogWording()
    {
        ...
        Assert.Contains("[\"관리자\", \"시그니처 슬로거\"]", authUser);
    ...
    public void UserFacingFailureMessagesUseLogAndRecallWording()
    {
        ...
        Assert.Contains("Share a Markdown Slogs log as a public log", postMcpTools);
        Assert.Contains("Optional owner-only pre-publish memory or public log slug", postMcpTools);
        Assert.Contains("Public log title", postMcpTools);
        ...
        Assert.Contains("Read an owned or public Slogs log-flow node by slug", postMcpTools);
        Assert.Contains("# Slogs Owner-Only Pre-Publish Memory Saved", postMcpTools);
        Assert.Contains("# Slogs Public-Sharing Node Shared", postMcpTools);
        Assert.Contains("# Slogs Log Flow Removed", postMcpTools);
        Assert.Contains("- Removed slug:", postMcpTools);
        Assert.Contains("The log is a public-sharing Slogs node, not an LLM Wiki memory.", postMcpTools);
        Assert.Contains("Slogs log MCP call", postMcpTools);
        Assert.Contains("Slogs 공개에는 로그 제목이 필요합니다.", postMcpTools);
        Assert.Contains("Slogs 공개에는 로그 Markdown 본문이 필요합니다.", postMcpTools);
        Assert.Contains("Slogs 게시전 로그 저장에는 제목이나 Markdown 본문 중 하나가 필요합니다.", postMcpTools);
        Assert.Contains("Flow state: {GetFlowState(post)}", postMcpTools);
        Assert.Contains("Former flow state: {GetFlowState(post)}", postMcpTools);
        Assert.Contains("Owner-only pre-publish memory", postMcpTools);
        Assert.Contains("Public log", postMcpTools);
        Assert.Contains("Public log time", postMcpTools);
        Assert.Contains("Flow refreshed", postMcpTools);
        ...
        Assert.DoesNotContain("Status:", postMcpTools);
        Assert.DoesNotContain("Former status:", postMcpTools);
        Assert.DoesNotContain("Publicly shared", postMcpTools);
        ...
        Assert.DoesNotContain("회상어가 필요", llmWikiService);
        Assert.Contains("검색어가 필요합니다.", llmWikiService);

        Assert.Contains("공개 기억 검색에는 @dimohy 같은 대상 슬로거 @name이 필요합니다.", llmWikiMcpTools);
    ...
    public void AdminUserFiltersUseClueLanguageInsteadOfGenericSearch()
    {
        ...
        Assert.Contains("<PageTitle>관리자 | slogs</PageTitle>", adminUsersPage);
        Assert.Contains(">관리자</h1>", adminUsersPage);
        Assert.DoesNotContain("운영 슬로거", adminUsersPage);
        Assert.Contains("aria-label=\"운영 메뉴\"", adminUsersPage);
        Assert.Contains("관리자 권한이 필요합니다.", adminUsersPage);
        Assert.Contains(">슬로거</a>", adminUsersPage);
        Assert.DoesNotContain(">슬로거 홈</a>", adminUsersPage);
        Assert.Contains(">LLM Wiki</a>", adminUsersPage);
        Assert.DoesNotContain(">LLM Wiki 지표</a>", adminUsersPage);
        Assert.Contains(">노트 Vault</a>", adminUsersPage);
        Assert.DoesNotContain("aria-label=\"운영 슬로거 보기\"", adminUsersPage);
        Assert.DoesNotContain(">노트 Vault 흐름</a>", adminUsersPage);
        Assert.Contains("aria-label=\"슬로거 요약\"", adminUsersPage);
        Assert.Contains(">등록 슬로거</div>", adminUsersPage);
        Assert.Contains(">로그</div>", adminUsersPage);
        Assert.Contains(">공개 로그 흐름</div>", adminUsersPage);
        Assert.Contains(">@@name 정리 후보</div>", adminUsersPage);
        Assert.Contains("placeholder=\"슬로거 태그\"", adminUsersPage);
        Assert.Contains("aria-label=\"슬로거 태그 입력\"", adminUsersPage);
        Assert.Contains("placeholder=\"LLM Wiki 슬로거 태그\"", adminUsersPage);
        Assert.Contains("aria-label=\"LLM Wiki 슬로거 태그 입력\"", adminUsersPage);
        Assert.Contains("placeholder=\"노트 Vault 슬로거 태그\"", adminUsersPage);
        Assert.Contains("aria-label=\"노트 Vault 슬로거 태그 입력\"", adminUsersPage);
        Assert.Contains("기억 슬로거만", adminUsersPage);
        Assert.Contains("노트 Vault 슬로거만", adminUsersPage);
        Assert.Contains("aria-label=\"슬로거 정렬\"", adminUsersPage);
        Assert.Contains("<option value=\"registered\">처음 연결순</option>", adminUsersPage);
        Assert.Contains("<option value=\"profile\">최근 갱신순</option>", adminUsersPage);
        Assert.Contains("<option value=\"posts\">로그순</option>", adminUsersPage);
        Assert.Contains("<option value=\"name\">@@name순</option>", adminUsersPage);
        Assert.Contains("명 표시", adminUsersPage);
        Assert.Contains(">처음 연결</th>", adminUsersPage);
        Assert.Contains(">홈 갱신</th>", adminUsersPage);
        Assert.Contains(">로그</th>", adminUsersPage);
        Assert.Contains(">공개 홈</th>", adminUsersPage);
        Assert.Contains(">@@name</th>", adminUsersPage);
        Assert.Contains("예약 @@name", adminUsersPage);
        Assert.Contains("표시할 슬로거가 없습니다.", adminUsersPage);
        ...
        Assert.Contains("slogs 슬로거, LLM Wiki, 노트 Vault를 함께 관리합니다.", adminUsersPage);
        Assert.Contains("슬로거 홈과 공개 로그, 게시전 로그이 어떻게 이어지는지 함께 관리합니다.", adminUsersPage);
        Assert.Contains("비공개 기억, 조회, Agent 연결 품질를 함께 확인합니다.", adminUsersPage);
        Assert.Contains("로컬 노트 Vault, 노트 원문, 연결 기록을 함께 관리합니다.", adminUsersPage);
        Assert.Contains(">기억</div>", adminUsersPage);
        Assert.Contains(">기억 활동</div>", adminUsersPage);
        Assert.Contains(">7일 기억 활동</div>", adminUsersPage);
        Assert.Contains(">30일 기억 활동</div>", adminUsersPage);
        Assert.Contains("aria-label=\"Agent 품질 지표\"", adminUsersPage);
        Assert.Contains(">LLM Wiki 품질</h2>", adminUsersPage);
        Assert.Contains("이후 감사 정렬", adminUsersPage);
        Assert.Contains("최근 Agent 접근", adminUsersPage);
        Assert.Contains(">30일 Agent 접근</div>", adminUsersPage);
        Assert.Contains(">후보</div>", adminUsersPage);
        Assert.Contains("후보 조회", adminUsersPage);
        Assert.Contains(">유효 결과율</div>", adminUsersPage);
        Assert.Contains("빈 결과", adminUsersPage);
        Assert.Contains(">응답 속도</div>", adminUsersPage);
        Assert.Contains(">반복 요청률</div>", adminUsersPage);
        Assert.Contains("느린 요청", adminUsersPage);
        Assert.Contains(">기억 변경</div>", adminUsersPage);
        Assert.Contains(">도구</th>", adminUsersPage);
        Assert.Contains(">접근</th>", adminUsersPage);
        Assert.Contains(">유효</th>", adminUsersPage);
        Assert.Contains(">최근 접근</th>", adminUsersPage);
        Assert.Contains("최근 Agent 감사 로그가 없습니다.", adminUsersPage);
        Assert.Contains(">슬로거</div>", navMenu);
        Assert.Contains(">슬로거 홈</a>", navMenu);
        Assert.Contains(">LLM Wiki</a>", navMenu);
        Assert.Contains(">노트 Vault</a>", navMenu);
        Assert.Contains("<option value=\"entries\">기억순</option>", adminUsersPage);
        Assert.Contains("<option value=\"accesses\">조회순</option>", adminUsersPage);
        ...
        Assert.Contains("명 기억 활동 표시", adminUsersPage);
        Assert.Contains(">기억</th>", adminUsersPage);
        Assert.Contains(">근거</th>", adminUsersPage);
        ...
        Assert.Contains(">7일 기억 활동</th>", adminUsersPage);
        Assert.Contains(">30일 기억 활동</th>", adminUsersPage);
        Assert.Contains(">조회</th>", adminUsersPage);
        ...
        Assert.Contains("공개 {user.PublishedPostCount:N0} / 게시전 로그 {user.DraftPostCount:N0}", adminUsersPage);
        ...
        Assert.DoesNotContain("어드민 슬로거", adminUsersPage);
        ...
        Assert.DoesNotContain(">공개 로그 흐름</div>", adminUsersPage);
        Assert.DoesNotContain("placeholder=\"사용자 태그\"", adminUsersPage);
        Assert.DoesNotContain("aria-label=\"사용자 태그 입력\"", adminUsersPage);
        Assert.DoesNotContain("placeholder=\"LLM Wiki 사용자 태그\"", adminUsersPage);
        Assert.DoesNotContain("aria-label=\"LLM Wiki 사용자 태그 입력\"", adminUsersPage);
        Assert.DoesNotContain("placeholder=\"Obsidian Sync 사용자 태그\"", adminUsersPage);
        Assert.DoesNotContain("aria-label=\"Obsidian Sync 사용자 태그 입력\"", adminUsersPage);
        Assert.DoesNotContain("placeholder=\"Obsidian Sync 슬로거 태그\"", adminUsersPage);
        Assert.DoesNotContain("aria-label=\"Obsidian Sync 슬로거 태그 입력\"", adminUsersPage);
        ...
        Assert.DoesNotContain("슬로거 홈, 공개 로그, 게시전 로그 정보를 확인합니다.", adminUsersPage);
        Assert.DoesNotContain("slogs 슬로거, LLM Wiki, 노트 Vault를 확인합니다.", adminUsersPage);
        Assert.DoesNotContain("슬로거 홈과 공개/게시전 로그가 어떻게 이어지는지 확인합니다.", adminUsersPage);
        Assert.DoesNotContain("슬로거 홈과 공개/게시전 로그가 어떻게 이어지는지 함께 관리합니다.", adminUsersPage);
        ...
        Assert.DoesNotContain("비공개 기억, 조회, Agent 연결 품질를 확인합니다.", adminUsersPage);
        Assert.DoesNotContain("MCP 호출 품질", adminUsersPage);
        Assert.DoesNotContain("aria-label=\"MCP 품질 지표\"", adminUsersPage);
        Assert.DoesNotContain("aria-label=\"MCP 검색 품질 지표\"", adminUsersPage);
        ...
        Assert.DoesNotContain("최근 MCP 검색 감사 로그가 없습니다.", adminUsersPage);
        ...
        Assert.DoesNotContain("게시전 로그 관리", adminUsersPage);
        Assert.DoesNotContain("공개 로그 {user.PublishedPostCount:N0} / 게시전 로그 {user.DraftPostCount:N0}", adminUsersPage);
        Assert.DoesNotContain("Obsidian Sync 노트 Vault, 노트 원문, 연결 기기 내용을 확인합니다.", adminUsersPage);
        ...
        Assert.DoesNotContain(">홈</th>", adminUsersPage);
    ...
    public void LlmWikiRecallCardsUseRecallAccessAndMemoryFlowWording()
    {
        ...
        Assert.Contains("기억 범주로 좁힌 뒤 아래 기억에서 이어 쓸 기억을 선택합니다.", llmWikiSearchPage);
        ...
        Assert.Contains("이어 쓸 기억", llmWikiSearchPage);
        Assert.Contains("data-llm-wiki-recall-flowline=\"true\"", llmWikiSearchPage);
        Assert.Contains("aria-label=\"LLM Wiki LLM Wiki 목록 정보\"", llmWikiSearchPage);
        Assert.Contains("현재 기억 검색 목록", llmWikiSearchPage);
        ...
        Assert.Contains("검토 후 공개", llmWikiSearchPage);
        Assert.Contains("검색된 기억", llmWikiSearchPage);
        Assert.Contains("게시전 로그 연결", llmWikiSearchPage);
        Assert.Contains("게시전 로그 연결 대기", llmWikiSearchPage);
        Assert.Contains("범주+검색", llmWikiSearchPage);
        Assert.Contains("개 LLM Wiki 중", llmWikiSearchPage);
        Assert.Contains("회 조회", llmWikiSearchPage);
        ...
        Assert.DoesNotContain("검색된 기억이 없습니다.", llmWikiSearchPage);
    ...
    public void LlmWikiMemoryToLogBridgeUsesPrePublishMemoryWording()
    {
        ...
        Assert.Contains("개인 LLM Wiki 기억을 검색하고 Slogs 게시전 로그로 이어 씁니다.", llmWikiSearchPage);
        ...
        Assert.DoesNotContain("개인 LLM Wiki 기억을 검색하고 Slogs 게시전 로그로 이어 씁니다.", llmWikiSearchPage);
        ...
        Assert.DoesNotContain("비공개 기억 -> 소유자 전용 게시전 로그 -> 검토 후 공개", llmWikiSearchPage);
        ...
        Assert.DoesNotContain("검토 후 공개 로그", llmWikiSearchPage);
        Assert.DoesNotContain("공개 후에는 슬로거 홈, 공개 로그, 댓글, 리비전으로 이어집니다.", llmWikiSearchPage);
        Assert.DoesNotContain("공개 후 목록: 슬로거 홈 -> 공개 로그 -> 댓글 -> 리비전 흐름", llmWikiSearchPage);
    ...
    public void LlmWikiDetailModalFramesRawProvenanceAsMemoryEvidenceFlow()
    {
        ...
        Assert.Contains("기억을 만든 요청", llmWikiSearchPage);
        Assert.Contains("정리된 기억 내용", llmWikiSearchPage);
        Assert.Contains("기억 근거", llmWikiSearchPage);
        Assert.Contains("기억으로 남기거나 병합/갱신할 때 남긴 근거를 공개 전 감사 로그로 다시 따라갑니다.", llmWikiSearchPage);
        ...
        Assert.Contains("근거 태그", llmWikiSearchPage);
        Assert.Contains("기억 범주", llmWikiSearchPage);
        Assert.Contains("근거 요청", llmWikiSearchPage);
        ...
        Assert.DoesNotContain("저장, 병합, 갱신 때 남긴 원문 근거를 공개 전 감사 로그로 확인합니다.", llmWikiSearchPage);
    ...
    public void LlmWikiUsageGuideFramesToolNamesAsRecallFlow()
    {
        ...
        Assert.Contains("한국어 Agent 지침", llmWikiGuidePage);
        Assert.Contains("English Recall Instructions", llmWikiGuidePage);
        Assert.Contains("Copy English Instructions", llmWikiGuidePage);
    ...
    public void SettingsPageFramesConnectionLayerAsKnowledgeLogFlow()
    {
        ...
        Assert.Contains("슬로거 홈 정체성, Agent, LLM Wiki 기억, Obsidian 노트 Vault, 공개을 한 연결 계층으로 이어 둡니다.", settingsPage);
        Assert.Contains("슬로거 홈 정체성, Agent, 기억, 로컬 노트, 공개을 한 연결 계층으로 이어 둡니다.", settingsPage);
        Assert.Contains("기억과 노트가 공개로 이어지는 경로", settingsPage);
        Assert.Contains("Agent는 비공개 기억을 검색하고, Obsidian은 로컬 노트를 원격 노트 Vault에 남기며", settingsPage);
        Assert.Contains("data-settings-connection-status", settingsPage);
        Assert.Contains("노트 Vault", settingsPage);
        Assert.Contains("소유자 전용 게시전 로그", settingsPage);
        Assert.Contains("비공개 기억을 검색해 소유자 전용 게시전 로그로 이어 둡니다.", settingsPage);
        Assert.Contains("판단 기준과 작업 맥락을 찾아 게시전 로그로 이어 씁니다.", settingsPage);
        Assert.Contains("frontmatter 태그를 게시전 로그과 LLM Wiki 기억으로 이어 둡니다.", settingsPage);
        Assert.Contains("게시전 로그 정리", settingsPage);
        Assert.Contains("검토된 기억과 노트를 게시전 로그로 정리하고 필요할 때 공개합니다.", settingsPage);
        Assert.Contains("이 슬로거의 비공개 기억을 검색하고", settingsComponent);
        Assert.Contains("검토 가능한 소유자 전용 게시전 로그를 만들고", settingsComponent);
        Assert.Contains("data-agent-mcp-flow-status", settingsComponent);
        Assert.Contains("<span>소유자 전용 게시전 로그</span>", settingsComponent);
        Assert.Contains("frontmatter를 통해 게시전 로그, 공개, LLM Wiki 기억으로 이어질 수 있습니다.", settingsComponent);
        Assert.Contains("<span>게시전 로그</span>", settingsComponent);
        Assert.Contains("공개는 명시 후", settingsComponent);
        Assert.Contains("Slogs MCP 연결 주소", settingsComponent);
        Assert.Contains("Agent 권한 헤더", settingsComponent);
        Assert.Contains("Agent 연결 설정 예시", settingsComponent);
        Assert.Contains("Agent는 Agent 권한으로 이 슬로거의 비공개 기억을 검색하고", settingsComponent);
        ...
        Assert.Contains("Slogs Drive 설치", settingsComponent);
        Assert.Contains("Slogs Drive 실행", settingsComponent);
        Assert.Contains("Slogs Drive 설치이 복사되었습니다.", settingsComponent);
        Assert.Contains("Slogs Drive 실행이 복사되었습니다.", settingsComponent);
        Assert.Contains("로컬 Markdown 노트는 노트 Vault 권한으로 원격 노트 Vault에 남고", settingsComponent);
        Assert.Contains("선택한 노트를 LLM Wiki 기억, 소유자 전용 게시전 로그, 공개 목록으로 이어갈 수 있습니다.", settingsComponent);
        Assert.Contains("노트 Vault 실험 빌드가 필요할 때만 GitHub Release 또는 BRAT 경로를 연결합니다.", settingsComponent);
        Assert.Contains("일반 노트은 Obsidian 커뮤니티 플러그인 Slogs Sync로 이어집니다.", settingsComponent);
        Assert.Contains("연결된 노트 Vault가 없습니다. 로컬 노트를 LLM Wiki 기억, 소유자 전용 게시전 로그, 공개 목록으로 이어 쓸 공간을 먼저 여세요.", settingsComponent);
        Assert.Contains("data-obsidian-flow-status", settingsComponent);
        Assert.Contains("frontmatter 태그", settingsComponent);
        ...
        Assert.Contains("Agent 권한", settingsComponent);
        ...
        Assert.Contains("공개 지식 로그 홈에 보일 이름, 이미지, 짧은 소개를 정리합니다.", profileSettingsForm);
        ...
        Assert.DoesNotContain("공개 로그를 관리합니다.", settingsPage);
        Assert.DoesNotContain("슬로거 홈 정체성, Agent, LLM Wiki 기억, Obsidian 노트 Vault, 공개 로그를 한 연결 계층으로 이어 둡니다.", settingsPage);
        Assert.DoesNotContain("슬로거 홈 정체성, Agent, 기억, 로컬 노트, 공개 로그를 한 연결 계층으로 이어 둡니다.", settingsPage);
        ...
        Assert.DoesNotContain("검토된 기억과 노트를 리비전으로 공개합니다.", settingsPage);
        ...
        Assert.DoesNotContain("비공개 기억을 검색해 소유자 전용 게시전 로그로 이어 둡니다.", settingsPage);
        Assert.DoesNotContain("판단 기준과 작업 맥락을 찾아 게시전 로그로 이어 씁니다.", settingsPage);
        Assert.DoesNotContain("frontmatter 태그를 게시전 로그와 LLM Wiki 기억으로 이어 둡니다.", settingsPage);
        Assert.DoesNotContain("검토 가능한 소유자 전용 게시전 로그를 만들고", settingsComponent);
        Assert.DoesNotContain("frontmatter를 통해 게시전 로그, 공개, LLM Wiki 기억으로 이어질 수 있습니다.", settingsComponent);
        Assert.DoesNotContain("선택한 노트를 Slogs 기억과 로그로 이어갈 수 있습니다.", settingsComponent);
    ...
    public void ObsidianVaultCardsUseNoteFlowAndConnectedDeviceWording()
    {
        ...
        Assert.Contains("노트 v{vault.CurrentVersion}", settingsComponent);
        ...
        Assert.Contains("노트 원문, 지운 노트 흔적, 연결 흔적, 노트 버전", settingsComponent);
        ...
        Assert.Contains("노트 v{client.LastSeenVersion}", settingsComponent);

        Assert.DoesNotContain(">v@vault.CurrentVersion ·", settingsComponent);
        Assert.DoesNotContain("노트 v@vault.CurrentVersion", settingsComponent);
        ...
        Assert.DoesNotContain("노트 v@client.LastSeenVersion", settingsComponent);
    ...
    public void ObsidianPluginFramesNotesAsMemoryAndSharingFlow()
    {
        ...
        Assert.Contains("Open mapped Slogs log", pluginMain);
        ...
        Assert.Contains("local Markdown notes can continue as note clues, LLM Wiki memory, owner-only pre-publish memory, and public logs", pluginReadme);
    ...
    public void AdminObsidianMetricsUseNoteVaultAndConnectionTraceWording()
    {
        ...
        Assert.Contains("노트 Vault 요약", adminUsersPage);
        ...
        Assert.Contains("aria-label=\"노트 Vault 용량\"", adminUsersPage);
        Assert.Contains(">노트 Vault 용량</h2>", adminUsersPage);
        Assert.Contains("슬로거 홈당 {FormatBytes(usage.ObsidianPerAccountStorageLimitBytes)} · 전체 Vault {FormatBytes(usage.ObsidianTotalStorageCapacityBytes)}", adminUsersPage);
        Assert.Contains(">노트 Vault 한도 GiB</label>", adminUsersPage);
        Assert.Contains("aria-label=\"노트 Vault 한도 GiB\"", adminUsersPage);
        Assert.Contains("한도 반영 중", adminUsersPage);
        Assert.Contains("한도 반영", adminUsersPage);
        Assert.Contains("노트 Vault 한도를 {FormatBytes(capacityBytes)}로 반영했습니다.", adminUsersPage);
        Assert.Contains("반영된 노트 Vault 한도 값이 올바르지 않습니다.", adminUsersPage);
        Assert.Contains("노트 Vault 한도 반영에 실패했습니다.", adminUsersPage);
        ...
        Assert.Contains(">전체 사용률</div>", adminUsersPage);
        Assert.Contains("노트 Vault 슬로거만", adminUsersPage);
        Assert.Contains("placeholder=\"노트 Vault 슬로거 태그\"", adminUsersPage);
        Assert.Contains("aria-label=\"노트 Vault 슬로거 태그 입력\"", adminUsersPage);
        Assert.Contains("aria-label=\"노트 Vault 정렬\"", adminUsersPage);
        Assert.Contains("<option value=\"updated\">최근 노트순</option>", adminUsersPage);
        ...
        Assert.DoesNotContain("Obsidian Sync 노트 요약", adminUsersPage);
        Assert.DoesNotContain("Obsidian Sync 노트 Vault 용량 한도", adminUsersPage);
        Assert.DoesNotContain("노트 Vault 한도를 {FormatBytes(capacityBytes)}로 저장했습니다.", adminUsersPage);
        ...
        Assert.DoesNotContain(">Vault 흐름 한도</th>", adminUsersPage);
        ...
        Assert.DoesNotContain("Obsidian Sync 노트 Vault, 노트 원문, 연결 기기 내용을 확인합니다.", adminUsersPage);
        Assert.DoesNotContain("로컬 노트 Vault, 노트 원문, 연결 기기 내용을 확인합니다.", adminUsersPage);
        Assert.DoesNotContain("로컬 노트 Vault, 노트 원문, 연결 기록을 확인합니다.", adminUsersPage);
    }
    ...
    public void AuthoringAddressDefaultsUseLogAndClueWording()
    {
        ...
        {
            Assert.Contains("공유 주소", authoringPage);
            Assert.Contains("제목으로 주소 만들기", authoringPage);
            Assert.DoesNotContain("공유 주소 slug", authoringPage);
            Assert.DoesNotContain("제목으로 주소 추천", authoringPage);
            Assert.DoesNotContain("제목으로 태그 추천", authoringPage);
        }
    }
    ...
    public void AuthoringDraftFlowUsesPrePublishAndShareLanguage()
    {
        ...
        Assert.Contains("<PageTitle>게시전 로그 남기기</PageTitle>", writePost);
        Assert.Contains("Title=\"게시전 로그 남기기 | slogs\"", writePost);
        Assert.Contains("생각, 작업, 판단, 검증을 게시전 로그로 남기고 필요할 때 공개합니다.", writePost);
        Assert.Contains(">게시전 로그 남기기</h1>", writePost);
        Assert.Contains("게시전 로그 저장", writePost);
        Assert.Contains("게시전 로그로 남길 로그 제목이나 본문을 입력해 주세요.", writePost);
        Assert.Contains("공개에는 로그 제목과 본문이 필요합니다.", writePost);
        Assert.Contains("공개 중...", writePost);
        Assert.Contains("공개", writePost);
        ...
        Assert.Contains("로그 정리", editPost);
        Assert.Contains("slogs 로그 내용을 게시전 로그 정리와 리비전 공유로 이어갑니다.", editPost);
        Assert.Contains("게시전 로그 정리", editPost);
        Assert.Contains("리비전 공유 정리", editPost);
        Assert.Contains(">게시전 로그</span>", editPost);
        Assert.Contains("게시전 로그 저장", editPost);
        Assert.Contains("정리할 수 있는 로그를 찾지 못했습니다.", editPost);
        Assert.Contains("이 로그 내용을 정리할 수 있는 연결 권한이 없습니다.", editPost);
        Assert.Contains("공개에는 로그 제목과 본문이 필요합니다.", editPost);
        Assert.Contains("리비전 공유에는 로그 제목과 본문이 필요합니다.", editPost);
        Assert.Contains("게시전 로그를 정리할 연결 권한이 없거나 로그가 없습니다.", editPost);
        Assert.Contains("리비전을 공유할 연결 권한이 없거나 로그가 존재하지 않습니다.", editPost);
        Assert.Contains("공개 중...", editPost);
        Assert.Contains("공개", editPost);
        ...
        Assert.Contains("업로드한 이미지는 로그 본문 Markdown에도 포함되어야 로그에 남습니다.", representativeImageField);
        Assert.Contains("로그에 남기려면 본문 Markdown에도 이 경로를 넣어 주세요.", representativeImageField);
    ...
    public void PersonalWorkspaceCardsUseLogFlowSignals()
    {
        ...
        Assert.Contains("공개 로그, 게시전 로그, 노트 Vault 내용이 모이는 슬로거 홈", program);
        Assert.DoesNotContain("공개 노드, 게시전 로그, 노트 Vault 흐름", program);
        Assert.DoesNotContain("글과 프로필", program);

        Assert.Contains("내 지식 로그", profilePage);
        Assert.Contains("공개 로그와 소유자 전용 게시전 로그를 이어 봅니다.", profilePage);
        Assert.Contains("생각, 작업, 판단, 검증을 먼저 게시전 로그로 남긴 뒤 필요할 때 공개하세요.", profilePage);
        Assert.Contains("@($\"{totalCount}개 로그\")", profilePage);
        Assert.Contains(">게시전 로그</span>", profilePage);
        Assert.Contains("수정 @FormatDate(post.UpdatedAt)", profilePage);
        Assert.Contains(">공개</span>", profilePage);
        Assert.Contains("게시전 로그 활동 검색", profilePage);
        Assert.Contains("공개 로그 검색", profilePage);
        Assert.DoesNotContain("게시전 로그 활동 열기", profilePage);
        Assert.DoesNotContain("공개 로그 열기", profilePage);
        Assert.Contains("<PostFlowSignals Post=\"post\" />", profilePage);
        Assert.Contains("게시전 로그 정리", profilePage);
        Assert.Contains("새 리비전 남기기", profilePage);
        Assert.Contains("로그 지우기", profilePage);
        Assert.Contains("로그 지우는 중...", profilePage);
        Assert.Contains("이 로그를 지울까요?", profilePage);
        ...
        Assert.Contains("이 로그를 지울까요?", postDetailsPage);
    ...
    public void PublicLogViewCountsUseRecallAccessWording()
    {
        ...
        Assert.Contains("title=\"조회\"", postMetaLine);
        Assert.Contains("조회, 댓글, 공감으로 다시 이어진 공개 로그를 따라갑니다.", homePage);
        ...
        Assert.DoesNotContain("조회, 대화 흔적, 공감으로", homePage);
        Assert.DoesNotContain("검색 진입", writerPage);
    }
    ...
    public void PublicLogActionsUseResonanceAndSavedRecallLanguage()
    {
        ...
        Assert.Contains("title=\"공감\"", postActionBar);
        Assert.Contains("<span class=\"sr-only\">공감</span>", postActionBar);
        Assert.Contains("공감 남기기, 현재 {Post.LikeCount}개", postActionBar);
        Assert.Contains("공감 해제, 현재 {Post.LikeCount}개", postActionBar);
        Assert.Contains("title=\"저장\"", postActionBar);
        Assert.Contains("<span class=\"sr-only\">저장</span>", postActionBar);
        Assert.Contains("저장에 추가", postActionBar);
        Assert.Contains("저장 해제", postActionBar);

        Assert.Contains("공감을 남기려면 지식 로그 홈으로 돌아가야 합니다.", postDetailsPage);
        Assert.Contains("게시전 로그는 공개 후 공감을 남길 수 있습니다.", postDetailsPage);
        Assert.Contains("공감가 처리 중입니다.", postDetailsPage);
        Assert.Contains("공감한 로그에서 해제되었습니다.", postDetailsPage);
        Assert.Contains("공감한 로그에 추가되었습니다.", postDetailsPage);
        Assert.Contains("저장한 로그를 바꾸려면 지식 로그 홈으로 돌아가야 합니다.", postDetailsPage);
        Assert.Contains("게시전 로그는 공개 후 저장에 추가할 수 있습니다.", postDetailsPage);
        Assert.Contains("저장이 처리 중입니다.", postDetailsPage);
        Assert.Contains("저장한 로그에서 해제되었습니다.", postDetailsPage);
        Assert.Contains("저장한 로그에 추가되었습니다.", postDetailsPage);
        ...
        Assert.DoesNotContain("공감는 로그인 후 남길 수 있습니다.", postDetailsPage);
        ...
        Assert.DoesNotContain("저장은 로그인 후 사용할 수 있습니다.", postDetailsPage);
        Assert.DoesNotContain("저장은 로그인 후 이용 가능합니다.", postDetailsPage);
        Assert.DoesNotContain("게시전 로그는 공개 후 공감을 남길 수 있습니다.", postDetailsPage);
        Assert.DoesNotContain("게시전 로그는 공개 후 저장에 추가할 수 있습니다.", postDetailsPage);
    ...
    public void PublicSloggerDiscoveryUsesLogHomeFlowSignals()
    {
        ...
        Assert.Contains("<PageTitle>슬로거 홈</PageTitle>", writerIndexPage);
        Assert.Contains("Title=\"슬로거 홈 | slogs\"", writerIndexPage);
        Assert.Contains("로그 홈을 검색합니다.", writerIndexPage);
        Assert.Contains(">슬로거 홈</h1>", writerIndexPage);
        Assert.Contains(">로그 목록순</a>", writerIndexPage);
        ...
        Assert.Contains("슬로거 홈 정보 경로: @{item.Author} -> 공개 로그", writerIndexPage);
        Assert.Contains("모든 슬로거 홈을 불러왔습니다.", writerIndexPage);
        Assert.Contains("공개 로그를 남긴 슬로거가 없습니다.", writerIndexPage);
        Assert.Contains("와 이어지는 슬로거 검색 내용이 없습니다.", writerIndexPage);
        Assert.Contains("aria-label=\"슬로거 홈 목록 정보\"", writerIndexPage);
        Assert.Contains("현재 홈", writerIndexPage);
        ...
        Assert.DoesNotContain("공개 로그를 남긴 슬로거가 없습니다.", writerIndexPage);
        Assert.DoesNotContain("슬로거 검색 결과가 없습니다.", writerIndexPage);
    }
    ...
    public void PublicClueAndSeriesDiscoveryUseFlowSortLabels()
    {
        ...
        Assert.Contains(">반복 태그</h1>", tagIndexPage);
        Assert.Contains("slogs의 반복 태그를 검색하며 이어지는 공개 로그를 다시 따라갑니다.", tagIndexPage);
        Assert.Contains(">태그명순</a>", tagIndexPage);
        Assert.Contains("모든 태그을 불러왔습니다.", tagIndexPage);
        Assert.Contains("aria-label=\"태그 목록 정보\"", tagIndexPage);
        Assert.Contains("현재 태그", tagIndexPage);
        Assert.Contains("와 이어지는 태그 검색 내용이 없습니다.", tagIndexPage);
        Assert.Contains("FormatClueCount(allTags.Count)", tagIndexPage);
        Assert.Contains("개 태그", tagIndexPage);
        Assert.Contains("GetClueRecallPath(item)", tagIndexPage);
        Assert.Contains("태그 정보 경로: #{item.Tag} -> 공개 로그", tagIndexPage);
        Assert.Contains(">로그 시리즈</h1>", seriesIndexPage);
        Assert.Contains("slogs의 로그 시리즈를 검색하며 시간과 의미로 이어진 내용을 다시 따라갑니다.", seriesIndexPage);
        Assert.Contains(">시리즈명순</a>", seriesIndexPage);
        Assert.Contains("모든 시리즈 내용을 불러왔습니다.", seriesIndexPage);
        Assert.Contains("aria-label=\"로그 시리즈 목록 정보\"", seriesIndexPage);
        Assert.Contains("현재 시리즈 목록", seriesIndexPage);
        Assert.Contains("와 이어지는 로그 시리즈 검색 내용이 없습니다.", seriesIndexPage);
        ...
        Assert.Contains("로그 시리즈 정보 경로: {item.Series} -> 공개 로그", seriesIndexPage);
        Assert.Contains("aria-label=\"태그 상세 목록 정보\"", tagPage);
        Assert.Contains("#@Tag 태그", tagPage);
        Assert.Contains("GetFlowStatusDescription()", tagPage);
        Assert.Contains("FormatPublicLogNodeCount(totalCount)", tagPage);
        Assert.Contains("개 공개 로그", tagPage);
        Assert.Contains("아직 #{Tag} 태그로 이어진 공개 로그가 없습니다.", tagPage);
        Assert.Contains("aria-label=\"로그 시리즈 상세 목록 정보\"", seriesPage);
        Assert.Contains("@Series 시리즈 검색 내용", seriesPage);
        Assert.Contains("GetFlowStatusDescription()", seriesPage);
        Assert.Contains("FormatPublicLogNodeCount(totalCount)", seriesPage);
        Assert.Contains("개 공개 로그", seriesPage);
        Assert.Contains("아직 {Series} 시리즈로 이어진 공개 로그가 없습니다.", seriesPage);

        Assert.DoesNotContain(">이름순</a>", tagIndexPage);
        Assert.DoesNotContain(">이름순</a>", seriesIndexPage);
        Assert.DoesNotContain("모든 태그를 불러왔습니다.", tagIndexPage);
        Assert.DoesNotContain("모든 로그 시리즈를 불러왔습니다.", seriesIndexPage);
        Assert.DoesNotContain("태그 결과가 없습니다.", tagIndexPage);
        Assert.DoesNotContain("로그 시리즈 검색 결과가 없습니다.", seriesIndexPage);
        Assert.DoesNotContain("slogs의 반복 태그를 검색하고 이어지는 공개 로그를 확인합니다.", tagIndexPage);
        Assert.DoesNotContain("slogs의 로그 시리즈를 검색하고 시간과 의미로 이어진 내용을 확인합니다.", seriesIndexPage);
        Assert.DoesNotContain("@($\"{totalCount}개 로그가 이 태그로 이어집니다.\")", tagPage);
    ...
    public void HomeFirstScreenExposesCompactKnowledgeLogFlowStatus()
    {
        ...
        Assert.Contains("aria-label=\"현재 지식 로그\"", homePage);
        Assert.Contains("현재 목록", homePage);
        ...
        Assert.Contains("개 로그", homePage);
        Assert.Contains(">반응 로그</a>", homePage);
        Assert.Contains("반응으로 이어진 로그", homePage);
        Assert.Contains(">검색</a>", homePage);
        Assert.Contains("의미가 이어진 로그", homePage);
        Assert.Contains(">최근 로그</a>", homePage);
        Assert.Contains("최근 공개 로그", homePage);
        Assert.Contains("검색", homePage);
        Assert.Contains("검색 결과 | slogs", homePage);
        Assert.Contains("slogs에서 {GetDisplayedQuery()}와 이어지는 검색 결과을 다시 따라갑니다.", homePage);
        Assert.Contains("와 이어지는 검색 결과이 없습니다.", homePage);
        Assert.Contains("와 이어지는 검색 결과이 없습니다.", homePage);
        ...
        Assert.DoesNotContain("검색 흐름 결과 | slogs", homePage);
        Assert.DoesNotContain("검색 결과가 없습니다.", homePage);
        Assert.DoesNotContain("slogs에서 {GetDisplayedQuery()} 검색 결과를 확인합니다.", homePage);
        Assert.DoesNotContain("추천", homePage);
        Assert.DoesNotContain("의미로 추천된 로그", homePage);
        Assert.DoesNotContain("이어 읽을 로그를 고릅니다.", homePage);
        Assert.DoesNotContain("사람과 AI가 이어 쓰는 지식 로그입니다.", homePage);
        Assert.DoesNotContain(">반응 로그 흐름</a>", homePage);
    ...
    public void PublicLogNodeCardsExposeRecallPathSignals()
    {
        ...
        Assert.Contains("DraftActionText { get; set; } = \"게시전 로그 정리\"", postLogCard);
        Assert.DoesNotContain("DraftActionText { get; set; } = \"게시전 로그 수정\"", postLogCard);
        Assert.Contains("Post.IsDraft ? \"게시전 로그\" : \"공개 로그\"", postFlowSignals);
        Assert.Contains("대화 흔적", postFlowSignals);
        Assert.Contains("공감", postFlowSignals);
        Assert.Contains("post-log-card__recall-path", postRecallPath);
        Assert.Contains("로그 정보 경로", postRecallPath);
        Assert.Contains("FormatRecallPath(Post)", postRecallPath);
        Assert.Contains("정보 경로:", postRecallPath);
        ...
        Assert.Contains("{post.CommentCount:N0} 대화 흔적 / {post.LikeCount:N0} 공감", postRecallPath);
        Assert.Contains("태그 없음", postRecallPath);
    ...
    public void PublicStreamDoneAndEmptyStatesKeepKnowledgeFlowLanguage()
    {
        ...
        Assert.Contains("현재 지식 로그를 모두 불러왔습니다.", homePage);
        Assert.Contains("에 이어지는 검색 로그가 없습니다.", homePage);
        Assert.Contains("이어진 로그가 없습니다.", homePage);
        Assert.Contains("aria-label=\"공개 로그 정보\"", postIndexPage);
        Assert.Contains("현재 공개 로그", postIndexPage);
        Assert.Contains("FormatPublicLogNodeCount(allPosts.Count)", postIndexPage);
        Assert.Contains("GetFlowScopeLabel()", postIndexPage);
        Assert.Contains("개 공개 로그", postIndexPage);
        Assert.Contains("검색으로 이어진 공개 로그가 없습니다.", postIndexPage);
        Assert.Contains("내 지식 로그를 모두 불러왔습니다.", profilePage);
        Assert.Contains("대표로 이어 줄 공개 로그가 아직 없습니다.", writerPage);
    ...
    public void WriterHomeHeroExposesKnowledgeFlowSummarySignals()
    {
        ...
        Assert.Contains("aria-label=\"슬로거 지식 로그 요약\"", writerPage);
        Assert.Contains("지식 로그 요약", writerPage);
        ...
        Assert.Contains("공개 로그", writerPage);
        Assert.Contains("공개 로그 {totalCount}개", writerPage);
        Assert.Contains("공개 로그로 쌓이기 전의 지식 로그 홈입니다.", writerPage);
        Assert.Contains("슬로거 홈 이미지", writerPage);
        Assert.Contains("슬로거 홈 &#64;주소", writerPage);
        Assert.Contains("대표 기억", writerPage);
        Assert.Contains("PostRecallPath Post=\"featuredPost\"", writerPage);
        Assert.Contains("대표 기억 경로", writerPage);
        Assert.Contains("writer-featured-recall-path", writerPage);
        Assert.Contains("시간순 로그", writerPage);
        Assert.Contains("대표 로그 연결", writerPage);
        Assert.Contains("주요 태그 #", writerPage);
        Assert.Contains("주요 시리즈", writerPage);
        Assert.Contains("생각과 작업 판단을 검색합니다.", writerPage);
    ...
    public void PostDetailFirstViewExposesKnowledgeNodeFlowSummary()
    {
        ...
        Assert.Contains("aria-label=\"로그 요약\"", postDetailsPage);
        Assert.Contains("로그 요약", postDetailsPage);
        ...
        Assert.Contains("게시전 지식 로그", postDetailsPage);
        Assert.Contains(">게시전 로그</span>", postDetailsPage);
        Assert.Contains(">공개 로그</span>", postDetailsPage);
        Assert.Contains("return $\"{targetPost.Title} {nodeState} | slogs\";", postDetailsPage);
        Assert.Contains("소유자 전용 게시전 로그", postDetailsPage);
        Assert.Contains("return $\"{nodeState}: {summary}", postDetailsPage);
        Assert.Contains("SeoMetadata.PublicLogNodeJsonLd", postDetailsPage);
        Assert.Contains("본문과 댓글은", postDetailsPage);
        Assert.Contains("이어진 지식 로그입니다.", postDetailsPage);
        Assert.Contains("주요 태그 #", postDetailsPage);
        Assert.Contains("로그 시리즈", postDetailsPage);
        Assert.Contains("리비전 v", postDetailsPage);
        Assert.Contains("개 관련 로그", postDetailsPage);
        Assert.Contains("관련 로그 대기", postDetailsPage);
        Assert.Contains("개 이전/다음 로그", postDetailsPage);
        Assert.Contains("이전/다음 로그 없음", postDetailsPage);
        Assert.Contains("개 댓글", postDetailsPage);
        Assert.Contains("수정 @FormatDateTime(post.UpdatedAt)", postDetailsPage);
        Assert.Contains("대화 흔적", postDetailsPage);
        Assert.Contains("관련 로그", postDetailsPage);
        Assert.Contains("이전 로그", postDetailsPage);
        Assert.Contains("다음 로그", postDetailsPage);
        Assert.Contains(">관련 로그</h3>", postDetailsPage);
        Assert.Contains("관련 로그", postDetailsPage);
        Assert.Contains("아직 이어진 관련 로그가 없습니다.", postDetailsPage);
        Assert.Contains("PostRecallPath Post=\"item\"", postDetailsPage);
        Assert.Contains("관련 로그 경로", postDetailsPage);
        Assert.Contains("PostRecallPath Post=\"previousPost\"", postDetailsPage);
        Assert.Contains("이전 로그 경로", postDetailsPage);
        Assert.Contains(">이전 로그</p>", postDetailsPage);
        Assert.Contains("이전 로그가 없습니다.", postDetailsPage);
        Assert.Contains("PostRecallPath Post=\"nextPost\"", postDetailsPage);
        Assert.Contains("다음 로그 경로", postDetailsPage);
        Assert.Contains(">다음 로그</p>", postDetailsPage);
        Assert.Contains("다음 로그가 없습니다.", postDetailsPage);
        ...
        Assert.DoesNotContain("관련 로그 열기", postDetailsPage);
        Assert.DoesNotContain("이전 로그 열기", postDetailsPage);
        Assert.DoesNotContain("다음 로그 열기", postDetailsPage);
        Assert.DoesNotContain("대화 흔적으로 이동", postDetailsPage);
        Assert.DoesNotContain("관련 로그 보기", postDetailsPage);
        Assert.DoesNotContain("이전 로그 이동", postDetailsPage);
        Assert.DoesNotContain("다음 로그 이동", postDetailsPage);
        Assert.DoesNotContain("이전 로그 이동", postDetailsPage);
        Assert.DoesNotContain("다음 로그 이동", postDetailsPage);
        Assert.DoesNotContain(">이전 로그 흐름</p>", postDetailsPage);
    ...
    public void PostDetailRevisionComparisonUsesRevisionFlowLanguage()
    {
        ...
        Assert.Contains("aria-label=\"리비전 정보\"", postDetailsPage);
        Assert.Contains("리비전 v", postDetailsPage);
        Assert.Contains("변경 내용", postDetailsPage);
        Assert.Contains("선택한 리비전이 이 로그의 기억 활동을 어떻게 갱신했는지 다시 따라갑니다.", postDetailsPage);
        ...
        Assert.Contains("첫 공개", blogService);
        ...
        Assert.DoesNotContain("변경 내용", postDetailsPage);
        Assert.DoesNotContain("목록 영역 {diff.Label}", postDetailsPage);
        Assert.DoesNotContain("변경 내용 기록 없음", postDetailsPage);
        Assert.DoesNotContain("개 변경 내용", postDetailsPage);
        ...
        Assert.DoesNotContain("선택한 리비전이 이 로그의 기억을 어떻게 갱신했는지 확인합니다.", postDetailsPage);
    ...
    public void SavedAndResonancePagesUseLogNodeCards()
    {
        ...
        Assert.Contains("저장한 로그", bookmarksPage);
        Assert.Contains("내가 저장한 slogs 로그를 다시 검색하고 이어 읽습니다.", bookmarksPage);
        Assert.Contains("저장한 로그를 한곳에 모아 다시 읽습니다.", bookmarksPage);
        Assert.Contains("모든 저장한 로그를 불러왔습니다.", bookmarksPage);
        Assert.Contains("저장한 로그에서 제거되었습니다.", bookmarksPage);
        Assert.Contains("저장한 로그에 추가되었습니다.", bookmarksPage);
        Assert.Contains("저장 해제", bookmarksPage);
        Assert.Contains("공감한 로그", likesPage);
        Assert.Contains("내가 공감한 slogs 로그를 다시 따라갑니다.", likesPage);
        Assert.Contains("내 판단에 남은 공감의 로그를 모아봅니다.", likesPage);
        Assert.Contains("새 공감순", likesPage);
        Assert.Contains("모든 공감한 로그를 불러왔습니다.", likesPage);
        Assert.Contains("공감한 로그에서 해제되었습니다.", likesPage);
        Assert.Contains("공감한 로그에 추가되었습니다.", likesPage);
        ...
        Assert.DoesNotContain("내가 공감한 로그를 모아봅니다.", likesPage);
        Assert.DoesNotContain("모든 공감 로그를 불러왔습니다.", likesPage);
        Assert.DoesNotContain("공감 해제 중...", likesPage);
        Assert.DoesNotContain("공감가 해제되었습니다.", likesPage);
        Assert.DoesNotContain("공감가 추가되었습니다.", likesPage);
    }
    ...
    public void PersonalWorkspaceEmptyStatesOfferKnowledgeLogNextActions()
    {
        ...
        Assert.Contains("아직 이어진 지식 로그가 없습니다.", profilePage);
        ...
        Assert.Contains("기억에서 검색", profilePage);
        Assert.DoesNotContain("아직 남긴 로그가 없습니다.", profilePage);

        Assert.Contains("아직 저장한 로그가 없습니다.", bookmarksPage);
        Assert.Contains("공개 로그에서 다시 이어 읽을 로그", bookmarksPage);
        Assert.Contains("href=\"/post\"", bookmarksPage);
        Assert.Contains("공개 로그", bookmarksPage);
        Assert.Contains("href=\"/tag\"", bookmarksPage);
        Assert.Contains("태그", bookmarksPage);
        Assert.DoesNotContain("저장 로그가 없습니다.", bookmarksPage);

        Assert.Contains("아직 공감 로그가 없습니다.", likesPage);
        Assert.Contains("공감", likesPage);
        Assert.Contains("href=\"/recommended\"", likesPage);
        Assert.Contains("검색", likesPage);
        Assert.Contains("href=\"/post\"", likesPage);
        Assert.DoesNotContain("공감 로그가 없습니다.", likesPage);
        Assert.DoesNotContain("추천", likesPage);
    }
    ...
    public void WriterConnectionPagesUseRelationshipFlowLanguage()
    {
        ...
        Assert.Contains("공개 로그 홈이 누구에게 이어지고, 어떤 슬로거의 기억을 다시 따라갈지 검색합니다.", writerConnectionsPage);
        ...
        Assert.DoesNotContain("어떤 슬로거의 기억 활동을 따라가는지 확인합니다.", writerConnectionsPage);
    ...
    public void HomeFeedAndPostDetailAuthorActionsUseRelationshipFlowLanguage()
    {
        ...
        Assert.Contains(">팔로우</a>", homePage);
        Assert.Contains("팔로우를 보려면 지식 로그 홈으로 돌아가야 합니다.", homePage);
        Assert.Contains("아직 팔로우한 슬로거의 공개 로그가 없습니다.", homePage);
        Assert.Contains("아직 팔로우한 슬로거의 공개 로그가 없습니다.", homePage);
        Assert.Contains("슬로거 찾기", homePage);
        Assert.Contains("팔로우에서", homePage);
        Assert.Contains("관계로 이어진 로그", homePage);
        Assert.Contains("팔로우한 슬로거 홈의 공개 로그", homePage);
        Assert.DoesNotContain("이어 둔 로그는 로그인 후 이용 가능합니다.", homePage);
        Assert.DoesNotContain(">이어 둔 로그</a>", homePage);
        Assert.DoesNotContain("이어 둔 로그에서", homePage);
        ...
        Assert.Contains("팔로우 {authorFollowerCount}개", postDetailsPage);
    ...
    public void PostDetailReplyFlowUsesContinuingConversationTraceLanguage()
    {
        ...
        Assert.Contains("대화 흔적 (@post.CommentCount)", postDetailsPage);
        Assert.Contains("이 로그에 댓글을 남기려면", postDetailsPage);
        Assert.Contains("placeholder=\"이 로그에 이어질 대화 흔적을 남겨주세요\"", postDetailsPage);
        Assert.Contains("대화 흔적 남기기", postDetailsPage);
        Assert.Contains("아직 이어진 대화 흔적이 없습니다. 첫 대화 흔적을 남겨 보세요.", postDetailsPage);
        Assert.Contains(">최신 흐름순</a>", postDetailsPage);
        Assert.Contains(">오래된순</a>", postDetailsPage);
        Assert.Contains("정렬: {GetCommentSortLabel(commentSortOrder)} · 대화 흔적 {commentTotalCount}개", postDetailsPage);
        Assert.Contains("모든 대화 흔적을 불러왔습니다.", postDetailsPage);
        Assert.Contains("대화 잇기", postDetailsPage);
        Assert.Contains("대화 흔적에 이어 남기기", postDetailsPage);
        Assert.Contains("대화 흔적 수정", postDetailsPage);
        Assert.Contains("대화 흔적 저장", postDetailsPage);
        Assert.Contains("대화 흔적 저장됨", postDetailsPage);
        Assert.Contains("대화 흔적 삭제", postDetailsPage);
        ...
        Assert.DoesNotContain(">최신 흐름순</a>", postDetailsPage);
    ...
    public void LocalRegistrationStartsWithSloggerProfileFields()
    {
        ...
        Assert.Contains("첫 슬로거 홈 태그", registerPage);
    ...
    public void LoginEntryUsesKnowledgeLogReturnLanguage()
    {
        ...
        Assert.Contains("내 비공개 기억, 게시전 로그, 저장한 로그와 공감한 로그", loginPage);
        Assert.Contains("슬로거 홈 @주소", loginPage);
        Assert.Contains("로그 목록으로 돌아가기", loginPage);
        ...
        Assert.Contains("지식 로그로 돌아가지 못했습니다.", loginPage);
        ...
        Assert.DoesNotContain("저장/공감", loginPage);
    ...
    public void PrivateLoginRequiredSurfacesUseKnowledgeLogReturnLanguage()
    {
        ...
        Assert.Contains("게시전 로그를 남기려면 내 지식 로그로 돌아가야 합니다.", writePage);
        Assert.Contains("게시전 로그를 남기려면 지식 로그 홈으로 돌아가야 합니다.", writePage);
        Assert.Contains("로그 내용을 정리하려면 내 지식 로그로 돌아가야 합니다.", editPage);
        Assert.Contains("내 공개 로그와 게시전 로그를 보려면 지식 로그 홈으로 돌아가야 합니다.", profilePage);
        Assert.Contains("저장한 로그를 다시 열려면 지식 로그 홈으로 돌아가야 합니다.", bookmarksPage);
    ...
    public void GoogleConfirmPageUsesSloggerHomeAddressFlowLanguage()
    {
        ...
        Assert.Contains("Google 계정에서 이어질 지식 로그 홈의 주소를 정해 주세요.", program);
        Assert.Contains("공개 로그, 게시전 로그, 노트 Vault 내용이 모이는 슬로거 홈", program);
        Assert.DoesNotContain("공개 노드, 게시전 로그, 노트 Vault 흐름", program);
        Assert.Contains("슬로거 홈 주소", program);
        Assert.Contains("홈 주소 잇기", program);
        Assert.Contains("연결 취소", program);
        Assert.Contains("슬로거 홈 주소를 입력해 주세요.", program);
    ...
    public void FailureRoutesUseKnowledgeLogRecoveryLanguage()
    {
        ...
        Assert.Contains("로그 내용을 찾지 못했습니다", notFoundPage);
        Assert.Contains("요청한 로그 내용을 찾지 못했습니다.", notFoundPage);
        Assert.Contains("지식 로그 홈", notFoundPage);
        Assert.Contains("공개 로그", notFoundPage);
        Assert.Contains("다시 찾는 경로", notFoundPage);
        Assert.Contains("태그나 슬로거", notFoundPage);
        ...
        Assert.Contains("요청을 처리하지 못했습니다", errorPage);
        Assert.Contains("요청을 처리하지 못했습니다.", errorPage);
        Assert.Contains("기억, 로그, 태그", errorPage);
        Assert.Contains("href=\"/write\">새 로그 남기기", errorPage);
        Assert.Contains("href=\"/me/llm-wiki/search\">검색", errorPage);
        Assert.Contains("href=\"/me\">내 지식 로그", errorPage);
        Assert.Contains("추적 ID", errorPage);
