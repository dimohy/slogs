...
public static class SlogsMcpPolicyPrompt
{
    ...
    public const string KoreanAgentsPolicyBlock = """
        ...
        - 코퍼스의 단일 정확 locator는 1홉, 문서·책 사이 한 관계 다리는 2홉, 실제 여러 단계 근거 사슬만 3홉을 사용한다. Retrieval Diagnostics의 `pairScoreCalls`와 `pairScoreCandidates`가 제공되면 중복 rerank와 지연을 확인하고, 같은 질문을 무작정 반복하거나 홉을 높여 비용을 숨기지 않는다.
    ...
    public const string EnglishAgentsPolicyBlock = """
        ...
        - Use 1 hop for one exact corpus locator, 2 hops for one relation bridge across documents or books, and 3 hops only for a genuinely multi-stage evidence chain. When Retrieval Diagnostics provides `pairScoreCalls` and `pairScoreCandidates`, inspect them for duplicate reranking and latency; do not hide cost by blindly repeating the query or increasing graph depth.
